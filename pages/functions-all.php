...
elseif($request[1]=="sales")
{
  ...
  foreach ($data as $k => $v) {
    if(isset($v[0])){
      ...
      $due+=$v[13];
      $nuom=$v[15];
      $nsql ="SELECT * from uom where id = '$nuom'";
      $nrow=$db->query($nsql)->fetch_assoc();
      $v[15]=$nrow['uom'];
      $val=[$v[1],$v[0],$v[3],$beats,$v[4],$v[6],$v[5],$v[8],$v[9],$v[10],$v[11],$v[12],$v[13],$v[14],$invoice,$v[2],$timestamp,$customer,$total,$discount,$v[15],$freight];
      $table="sales";
      $col= [
      'batch',
      'product',
...
elseif($request[1]=="purchasereportget")
{
  ...
  while($row=$result->fetch_assoc()){
    ...
    echo "<tr><td style='width:10px;'>".$i++."</td><td style='width:40px;'>$date</td><td >".$row['invoicenumber']."</td><td style='width:10px;'>".$supplier."</td><td style='width:10px;'>".$suppliercon."</td><td style='width:10px;'>".$row['totalwhole']."</td><td style='width:10px;'>".$row['sgst']."</td><td style='width:10px;'>".$row['cgst']."</td><td style='width:10px;' ><a href='../invoice/purchase/".$row['id']."'>View/Edit</a></td></tr>";
  }
}
